...
function Visit(schema: TSchema, analysis: SchemaAnalysis): void {
	...
	switch (schema[Kind]) {
		case 'Array': {
			FromArray(schema as TArray, analysis)
			return
		}
		case 'Intersect': {
			FromIntersect(schema as TIntersect, analysis)
			return
		}
		case 'Object': {
			FromObject(schema as TObject, analysis)
			return
		}
		case 'Record': {
			FromRecord(schema as TRecord, analysis)
			return
		}
		case 'Ref': {
			FromRef(schema as TRef, analysis)
			return
		}
		case 'Tuple': {
			FromTuple(schema as TTuple, analysis)
			return
		}
		case 'Union': {
			FromUnion(schema as TUnion, analysis)
			return
		}
	}
}
