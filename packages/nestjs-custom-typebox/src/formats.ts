...
const DAYS = [0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31] as const
const DATE = /^(\d\d\d\d)-(\d\d)-(\d\d)$/

const isLeapYear = (year: number): boolean => {
	return year % 4 === 0 && (year % 100 !== 0 || year % 400 === 0)
}
export const dateFormat = (value = ''): boolean => {
	...
	return (
		month >= 1 && month <= 12 && day >= 1 && day <= (month === 2 && isLeapYear(year) ? 29 : (DAYS[month] as number))
	)
}

const timeRegex = /^(\d\d):(\d\d):(\d\d(?:\.\d+)?)(z|([+-])(\d\d)(?::?(\d\d))?)?$/i
export const timeFormat = (value = '', strictTimeZone?: boolean): boolean => {
