...
import { applyDecorators, assignMetadata, Delete, Get, HttpCode, Patch, Post, Put } from '@nestjs/common'
...
import { Object, Static, StaticDecode, String as TString, TypeGuard, type TSchema } from '@sinclair/typebox'
...
import { capitalize, genGlobalResponse, pascalCase } from './util.js'
...
export function buildSchemaValidator(config: Omit<SchemaValidatorConfig, 'httpMessage'>): SchemaValidator {
...
export function Validate<
	...
	MethodDecoratorType extends (
		...args: [...RequestConfigsToTypes<RequestValidators>, ...unknown[]]
...
>(validatorConfig: ValidatorConfig<T, RequestValidators>): MethodDecorator<MethodDecoratorType> {
	return (target, key, descriptor) => {
		extendArrayMetadata(
			...
			descriptor.value as MethodDecoratorType,
		)

		const {
			default: defaultResponseValidatorConfig,
			request: requestValidatorConfigs,
			responses: responsesValidatorConfig,
		} = validatorConfig

		const methodName = capitalize(String(key))

		const swaggerResponseSchema: Record<
			number,
			{
		...
		if (defaultResponseValidatorConfig) {
			const validatorConfig: DefaultResponseValidatorConfig = TypeGuard.IsSchema(defaultResponseValidatorConfig)
				? { schema: defaultResponseValidatorConfig }
				: defaultResponseValidatorConfig

			const {
				httpMessage = 'OK',
				description = HttpStatusDescription[HttpStatus[httpMessage]],
				example,
				required = true,
			...
			swaggerResponseSchema[HttpStatus[httpMessage]] = { type: validator, description, example }
		}

		if (responsesValidatorConfig?.length) {
			for (const responseValidatorConfig of responsesValidatorConfig) {
				const {
					httpMessage,
					description = HttpStatusDescription[HttpStatus[httpMessage]],
					example,
					required = true,
					stripUnknownProps = true,
					name = `${methodName}${pascalCase(httpMessage)}`,
					schema = genGlobalResponse(),
					...config
		...
		Reflect.defineMetadata(
			...
			(target as Record<string | symbol, unknown>)[key] as object,
		)

		let args = (Reflect.getMetadata(ROUTE_ARGS_METADATA, target.constructor, key) ?? {}) as unknown as Metadata
		requestValidatorConfigs?.forEach((validatorConfig, index) => {
			switch (validatorConfig.type) {
				case 'body': {
					...
					const validatorPipe: PipeTransformTypeBox = {
						transform: (value) => validator.validate(value),
					}
					...
					ApiBody({ type: validator as unknown as NestType<unknown>, required })(target, key, descriptor)
				...
				case 'param': {
					...
					Reflect.defineMetadata(ROUTE_ARGS_METADATA, args, target.constructor, key)
					ApiParam({ name: validatorConfig.name, schema: validatorConfig.schema, required })(
						target,
						key,
						descriptor,
					)
				...
				case 'query': {
					...
					const validatorPipe: PipeTransformTypeBox = {
						transform: (value) => {
							return validator.validate(qs.parse(value as unknown as string))
						},
					}
					...
					Reflect.defineMetadata(ROUTE_ARGS_METADATA, args, target.constructor, key)
					ApiQuery({ name: validatorConfig.name, schema: validatorConfig.schema, required })(
						target,
						key,
						descriptor,
					)
...
export const HttpEndpoint = <
	...
	MethodDecoratorType extends (
		...args: [...RequestConfigsToTypes<RequestConfigs>, ...unknown[]]
...
>(
	...
	const decorators: MethodDecorator[] = [
		nestHttpDecoratorMap[method](path),
	...
	if (path) {
		...
		for (const pathParam of pathParams) {
			...
			if (paramValidator.required === false && pathParam.required) {
		...
		if (missingPathParam) {
			throw new Error(
				`Request validator references non-existent path parameter "${missingPathParam.name ?? ''}".`,
			)
		}
	}
